...
class MainScreen(Screen):
    def process_new_transcription(self, text, structured_data=None):
        app = App.get_running_app()
        
        # Create entry with structured data if available
        if structured_data and 'medicalNoteStructure' in structured_data:
            medical_structure = structured_data['medicalNoteStructure']
            
            # Extract progress notes
            progress_notes = medical_structure.get('progressNote', [])
            progress_text = progress_notes[0].get('context', text) if progress_notes else text
            
            # Extract observations
            observations = medical_structure.get('patientObservations', [])
            observation_text = observations[0].get('otherObservations', 'No observations') if observations else 'No observations'
            
            # Extract medications
            medications = medical_structure.get('medicationAdministered', [])
            medication_text = f"{medications[0].get('medicineType', 'No medication')} - {medications[0].get('quantity', '')}" if medications else 'No medication'
...
class MedicalApp(App):
    ...
    def on_start(self):
        """Initialize app after UI is built"""
        print("🚀 App starting...")
        self.load_session_from_json()
        
        # Schedule the initialization to happen after the UI is ready
        Clock.schedule_once(self.initialize_transcription_system, 1)
    
    def initialize_transcription_system(self, dt):
        """Initialize transcription system after UI is ready"""
        try:
            ...
            transcription_panel.test_backend_connection()
            
            # Start monitoring for new recordings
            transcription_panel.monitor_processed_recordings()
            
            # Store reference for later use - THIS IS IMPORTANT
            self.transcription_panel = transcription_panel
            
            print("✅ Transcription system initialized successfully")
            print("🔗 UI panels will connect to this monitoring system")
            
        except Exception as e:
            print(f"❌ Error initializing transcription system: {e}")
            import traceback
            traceback.print_exc()
