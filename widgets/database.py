    ...
    def save_transcription_session(tids, session_data: Dict[str, Any]) -> str:
        ...
        if not exists:
            try:
                ...
                if session_data["medicalNoteStructure"]["medicationAdministered"]["medicineType"] is not None:
                    cursor.execute('''
            ...
            ''', (tids, 
                  ...
                  session_data["medicalNoteStructure"]["progressNote"]["context"],
                    ...
                    #Save patient observation
            if session_data["medicalNoteStructure"]["patientObservation"]["otherObservations"] is not None:
                cursor.execute('''
                INSERT INTO patient_observation 
                (transcriptionId, timestamp, ObservationNote)
                VALUES (?, ?, ?)
